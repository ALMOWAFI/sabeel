...
class TensorflowService {
  ...
  public async getPersonalizedRecommendations(options: { 
    ...
    try {
      ...
      const response = await axios.post(`${API_ENDPOINTS.RECOMMENDATIONS}/personalized`, {
  ...
  private async getGeneralRecommendations(limit: number = 10): Promise<RecommendationItem[]> {
    try {
      const response = await axios.get(`${API_ENDPOINTS.RECOMMENDATIONS}/general?limit=${limit}`);
      return response.data.recommendations;
    } catch (error) {
  ...
  public async generatePersonalizedLearningPath(topic: string): Promise<LearningPath> {
    ...
    try {
      const response = await axios.post(`${API_ENDPOINTS.RECOMMENDATIONS}/learning-path`, {
        topic,
        userLevel: this.userLevel,
        userInterests: this.userInterests,
  ...
  public async getLearningPaths(limit: number = 5): Promise<LearningPath[]> {
    ...
    try {
      const response = await axios.get(`${API_ENDPOINTS.RECOMMENDATIONS}/learning-paths?limit=${limit}`);
  ...
  public async semanticSearch(query: string, options: {
    ...
    try {
      ...
      const response = await axios.post(`${API_ENDPOINTS.TEXT_ANALYSIS}/semantic-search`, {
  ...
  public async findRelatedContent(text: string, limit: number = 5): Promise<any[]> {
    ...
    try {
      const response = await axios.post(`${API_ENDPOINTS.TEXT_ANALYSIS}/related-content`, {
        text,
        limit
      });
